...
def prompt_filter(input_list, shape, sem):
    if sem == 'not-universal':
        filtered_targets = []
        for item in input_list:
            if 'nuni' in item:
                if (shape in item):
        ...
        selected_nuni = random.choice([item for item in input_list if 'nuni' in item])
        filtered_targets = [item for item in input_list if 'nex' in item]
        filtered_targets.append(selected_nuni)
        random.shuffle(filtered_targets)
        return filtered_targets
...
def enter_phase(client_id, phase):
    ...
    if phase == 'Start':
        ...
        if len(unpaired_clients) % 2 == 0:
            ...
            target_list1 = random.sample(trial_bank, len(trial_bank))
            target_list2 = random.sample(trial_bank, len(trial_bank))
            split_targets1 = split_trials(target_list1)
            split_targets2 = split_trials(target_list2)

            i = 0
            while i < len(split_targets1):
                item = split_targets1[i]
                if item['semantics'] == 'not-exist':
                    if item['lexicalization'] == 'compositional':
                        split_targets1.insert(i+1, item.copy())
                        i += 1
                i += 1
            
            i = 0
            while i < len(split_targets2):
                item = split_targets2[i]
                if item['semantics'] == 'not-exist':
                    if item['lexicalization'] == 'compositional':
                        split_targets2.insert(i+1, item.copy())
                        i += 1
                i += 1
...
def start_interaction_trial(list_of_participants):
    ...
    if not(all_connected(list_of_participants)):
        ...
        if trial_counter >= ntrials:
            ...
            shape = 'circle' if 'circle' in target['picture'] else 'tri'
...
def handle_director_response(director_id, director_response):
    ...
    if 'circle' in trial['picture']:
        director_shape = 'circle' 
    else:
        director_shape = 'tri'
