...
function generateBiasedObservationTrials(trialBank) {
  // Define two hand-crafted square items
  const redSquare = {
    lexicalization_number: 2,
    lexicalization: "compositional",
    sentence: "Not everything in this picture is red",
    semantics: "not-universal",
    picture: "square-nuni-red",
    shape: "square"
  };
  ...
  const blueSquare = {
    lexicalization_number: 2,
    lexicalization: "compositional",
    sentence: "Not everything in this picture is blue",
    semantics: "not-universal",
    picture: "square-nuni-blue",
    shape: "square"
  };

  // Take the first 6 trials
  trialBank = trialBank.slice(0, 6);

  // Separate into red and blue trials
  const redTrials = trialBank.filter(t => t.sentence.includes("red"));
  const blueTrials = trialBank.filter(t => !t.sentence.includes("red"));

  // SeparatePictures clone
  function splitTrials(data) {
    const output = [];
    data.forEach(item => {
      if (item["circle picture"]) {
        const circle = { ...item };
        circle.picture = item["circle picture"];
        circle.shape = "circle";
        output.push(circle);
      }
      if (item["triangle picture"]) {
        const triangle = { ...item };
        triangle.picture = item["triangle picture"];
  ...
  let splitRed = splitTrials(redTrials);
  let splitBlue = splitTrials(blueTrials);

  splitRed.push(redSquare);
  splitBlue.push(blueSquare);

  // Filter: remove triangle "Not everything..." items
  splitRed = splitRed.filter(item =>
    !(item.shape.toLowerCase().includes("triangle") &&
      item.sentence.startsWith("Not everything"))
  );

  splitBlue = splitBlue.filter(item =>
    !(item.shape.toLowerCase().includes("triangle") &&
      item.sentence.startsWith("Not everything"))
  );

  // Duplicate items with a frequency bias
  function multiplyTrials(trials) {
    const output = [];
    trials.forEach(trial => {
      let n = 6;
      let count = 0;
      if (trial.sentence.startsWith("Nothing")) {
        count = trial.shape === "triangle" ? n : Math.floor(n / 3);
      } else if (trial.sentence.startsWith("Everything")) {
        if (trial.shape === "triangle") {
          count = trial.semantics === "not-exist" ? n : Math.floor(n / 2);
        } else {
          count = trial.semantics === "not-exist" ? Math.floor(n / 3) : Math.floor(n / 2);
        }
      } else if (trial.shape === "square") {
        count = 4;
      }

      for (let i = 0; i < count; i++) {
        output.push({ ...trial }); // Deep copy
      }
    });
    return output;
  }

  const multBlue = multiplyTrials(splitBlue);
  const multRed = multiplyTrials(splitRed);

  // Interleave blue and red trials
  const maxLen = Math.max(multBlue.length, multRed.length);
  const shuffledTargets = [];

  for (let i = 0; i < maxLen; i++) {
    if (i < multBlue.length) shuffledTargets.push(multBlue[i]);
    if (i < multRed.length) shuffledTargets.push(multRed[i]);
  }
