...
// var object_4_labels = ["zop", "zopekil"];
// var object_5_labels = ["zop", "zopudon"];
var epiImp_labels = ["zop", "not lem"];
var deonImp_labels = ["zop", "not lem"];
// var epiNonnec_labels = ["lem not"];
// var deonNonnec_labels = ["lem not"];
var epiNonnec_labels = ["lem not", "not lem"];
var deonNonnec_labels = ["lem not", "not lem"];
// Wataru: the actual labels have to be sentences. 
...
var possible_objects = ["epiImp", "epiNonnec", "deonImp", "deonNonnec"];
...
var observation_trial_epiImp_lex = make_observation_trial("epiImp", "zop");
var observation_trial_epiImp_comp = make_observation_trial("epiImp", "not lem");
var observation_trial_deonImp_lex = make_observation_trial("deonImp", "zop");
var observation_trial_deonImp_comp = make_observation_trial("deonImp", "not lem");
var observation_trial_epiNonnec = make_observation_trial("epiNonnec", "lem not");
var observation_trial_deonNonnec = make_observation_trial("deonNonnec", "lem not");
...
Wataru => don't think we need frequency difference
*/
...
// Wataru: do we need variation in the contexts/pictures? 
// At some point, we need to introduce the weighting of the scores and tell participants about it. 
...
Wataru: need to check how this is done in practice
...
function director_trial(target_object, partner_id) {
  ...
  // }
    if (target_object == "epiImp") {
    label_choices = epiImp_labels; 
  } else if (target_object == "epiNonnec") {
    label_choices = epiNonnec_labels;
  } else if (target_object == "deonImp") {
    label_choices = deonImp_labels;
  } else if (target_object == "deonNonnec") {
    label_choices = deonNonnec_labels;
  }
  ...
  // Wataru: we don't need this component
